        ...
        static void Encajar(CompendiumSectionChampUpgrades seccion)
        {
            try
            {
                ...
                for (int i = 0; i < total; i++)
                {
                    ...
                    var boton = botones[i];
                    // transform.position apunta al PIVOTE del rombo, que no tiene por que
                    // estar en su centro: se corrige con el pivote, o la rejilla entera sale
                    // desplazada media celda.
                    float dx = 0f, dy = 0f;
                    if (boton is RectTransform brt)
                    {
                        dx = (brt.pivot.x - 0.5f) * rombo;
                        dy = (brt.pivot.y - 0.5f) * rombo;
                    }
                    boton.position = zona.TransformPoint(
                        new Vector3(x0 + col * pasoX + dx, y0 - fila * pasoY + dy, 0f));
